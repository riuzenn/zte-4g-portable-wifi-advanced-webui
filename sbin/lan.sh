...
br_set()
{
    ...
    IFS_OLD=$IFS
    IFS="+"
    ifconfig usblan0 hw ether 02:00:11:11:11:11
...
lan_mac_set()
{
	mac_tmp=`nv get at_wifi_mac`
	if [ "$mac_tmp" != "" ] ; then
		mac_p1=`echo $mac_tmp | cut -d ':' -f 1`
		echo "$mac_p1"
		if [ "$mac_p1" == "+MAC" ] ; then
			mac_p2=`echo $mac_tmp | cut -d ':' -f 2`		
			echo "$mac_p2"
			first=`echo $mac_p2 | cut -c 1-2`
			second=`echo $mac_p2 | cut -c 3-4`
			third=`echo $mac_p2 | cut -c 5-6`
			fourth=`echo $mac_p2 | cut -c 7-8`
			fiveth=`echo $mac_p2 | cut -c 9-10`
			sixth=`echo $mac_p2 | cut -c 11-12`
			lan_mac=$first:$second:$third:$fourth:$fiveth:$sixth
			echo "lan mac is :$lan_mac"
			ifconfig $br_name hw ether $lan_mac &
		fi
	fi
}
